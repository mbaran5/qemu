...
const NV = {
    ...
    addInput(settingsList, label, name, defaultVal = null, type = 'text', title = null) {
        ...
        NVUI.initSetting(name, defaultVal); NVUI.saveSetting(name);
...
const AudioPlugin = {
    ...
    async startAudio() {
        ...
        const codec = NVUI.getSetting('audio_codec') || 'opus';
        const bitrate = NVUI.getSetting('audio_bitrate') || '96000';
        const samplerate = NVUI.getSetting('audio_samplerate') || '48000';
        let mime;
        switch (codec) {
            case 'opus':
    ...
    initUi() {
        ...
        NV.addInput(audioSettings, 'Enabled', 'audio_enabled', true, 'checkbox', 'Enable audio plugin');
...
window.addEventListener('load', () => setTimeout(() => AudioPlugin.load(), 0));
